...
- в репозитории есть `.npmrc` с `omit=dev`, поэтому кнопка `NPM Install` в Plesk ставит только runtime-зависимости
- это нужно, чтобы Plesk не пытался собирать локальные build-пакеты вроде `ttf2woff2`, `gulp` и `esbuild`, которые на сервере не нужны
- для локальной разработки используйте `npm install --include=dev`
